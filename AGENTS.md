...
| 条目已写入，**整包待目标环境验证** | `web_multi_tabs` `19.0.2.0.0`、`product_variant` `19.0.13.0.4`、`web_list_refresh` `19.0.1.0.0` | 升级后需在中文「应用」列表里核对标题 / 摘要 / 描述 / 分类 ；`19.0.7.0.1` 修字段 widget 注册（此前产品表单打不开）；`19.0.7.0.2` 修模板变量名；`19.0.7.0.3` 修映射表排版；`19.0.7.0.4` 修「Add a line 即报必填缺失」；`19.0.7.0.5` 修 onchange 崩溃（映射状态改前端按需取）；`19.0.8.0.0` 映射计算搬到前端（编辑期零 RPC）；`19.0.8.0.1` 修「加属性面板无反应」；`19.0.8.0.2` 修「加属性无反应」；`19.0.8.0.3` 修「面板不刷新」；`19.0.8.0.4` 修「默认值当成用户选择」；`19.0.9.0.0` 组合唯一约束；`19.0.10.0.0` 映射表反向（组合为行、选变体）；`19.0.11.0.0` 所有属性一律穷举；`19.0.12.0.0` 前端穷举、后端按需；`19.0.12.0.1` 下拉禁用已占变体；`19.0.13.0.0` 面板标题与显式保存/丢弃；`19.0.13.0.1` 标题改「变体映射」；`19.0.13.0.2` 修「Save manually 点了没反应」（保存钩子里不能调 `getChanges()`）+ 改属性不再发多余 onchange + 面板下方不放按钮；`19.0.13.0.3` 删除唯一属性后自动复用既有变体；`19.0.13.0.4` 删唯一属性时守卫放行（带映射保存）|
