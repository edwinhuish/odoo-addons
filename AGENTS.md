...
| `product_image` | 19.0.2.0.8 | 产品多图（原生主图独立 + 图库补充图 / 删除主图自动提升（无替换按钮）/ 首张上传即主图 / 主图 2 倍 / 悬浮局部放大（放大镜跟随鼠标）/ 点击预览（放大缩小旋转）/ 右侧竖排缩略图（蓝色选中边框·删除提升·图库删除·滚动·无tooltip）/ 新增图片弹窗（顶层overlay·点击·拖放·Ctrl+V 粘贴后关闭）/ 继承 image.mixin 复用多尺寸；原名 `product_multi_image`） | 已交付，待目标环境验证（T-004） |
