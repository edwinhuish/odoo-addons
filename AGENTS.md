...
| `product_image` | 19.0.2.0.1 | 产品多图（原位多图浏览：不新增页签 / 主图 2 倍 / 悬浮放大 / 点击预览（放大缩小复制）/ 右侧竖排缩略图（删除/滚动/上传占位）/ 粘贴新增 / 首图即主图，继承 image.mixin 复用多尺寸；原名 `product_multi_image`） | 已交付，待目标环境验证（T-004） |
