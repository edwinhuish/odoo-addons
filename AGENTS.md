...
| `product_image` | 19.0.2.0.4 | 产品多图（原生主图独立 + 图库补充图：主图作为浏览序列首张，图库按序其后 / 删除主图自动提升图库首张 / 首张上传即主图 / 主图 2 倍 / 悬浮放大 / 点击预览（放大缩小旋转）/ 右侧竖排缩略图（主图替换·删除提升·图库删除·滚动·上传占位）/ 粘贴新增 / 继承 image.mixin 复用多尺寸；原名 `product_multi_image`） | 已交付，待目标环境验证（T-004） |
