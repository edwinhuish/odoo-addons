...
**落地情况**：仓库**全部 10 个模块**均已按本节写入这 4 类条目（`task check` 的「应用列表元数据一致性」检查覆盖全仓库，缺任一
条会告警）。按验收状态分：
...
| 条目已写入，**整包待目标环境验证** | `web_multi_tabs` `19.0.2.0.0`、`product_variant` `19.0.13.0.6`、`web_list_refresh` `19.0.1.0.0` | 升级后需在中文「应用」列表里核对标题 / 摘要 / 描述 / 分类 ；`19.0.7.0.1` 修字段 widget 注册（此前产品表单打不开）；`19.0.7.0.2` 修模板变量名；`19.0.7.0.3` 修映射表排版；`19.0.7.0.4` 修「Add a line 即报必填缺失」；`19.0.7.0.5` 修 onchange 崩溃（映射状态改前端按需取）；`19.0.8.0.0` 映射计算搬到前端（编辑期零 RPC）；`19.0.8.0.1` 修「加属性面板无反应」；`19.0.8.0.2` 修「加属性无反应」；`19.0.8.0.3` 修「面板不刷新」；`19.0.8.0.4` 修「默认值当成用户选择」；`19.0.9.0.0` 组合唯一约束；`19.0.10.0.0` 映射表反向（组合为行、选变体）；`19.0.11.0.0` 所有属性一律穷举；`19.0.12.0.0` 前端穷举、后端按需；`19.0.12.0.1` 下拉禁用已占变体；`19.0.13.0.0` 面板标题与显式保存/丢弃；`19.0.13.0.1` 标题改「变体映射」；`19.0.13.0.2` 修「Save manually 点了没反应」（保存钩子里不能调 `getChanges()`）+ 改属性不再发多余 onchange + 面板下方不放按钮；`19.0.13.0.3` 删除唯一属性后自动复用既有变体；`19.0.13.0.4` 删唯一属性时守卫放行（带映射保存）；`19.0.13.0.5` 面板说明/警告自动换行；`19.0.13.0.6` 文档同步 + 原生直写安全不变量用例|
...
- **x2many 的增删改一律走列表官方 API，`record.id` 不是命令 id**（`product_image` `19.0.2.6.6` 生产事故）：
  未保存的新行标识是 `record._virtualId`（`"virtual_3"`），`record.id` 是 **datapoint 内部 id**
  （`DataPoint` 构造里 `this.id = getId("datapoint")` → `"datapoint_12"`）。自己拼
  `x2ManyCommands.unlink(record.id)` 会：① 前端匹配不到那条待发的 `(0, 0, …)` 新建命令
  （`StaticList._applyCommands` 按 id 找 `_commands`）→ 新行留在列表（界面「同图重复」）、删除无反应；
  ② 保存时把 `(3, "datapoint_12")` 原样发出，服务端 `One2many.write_real` 的 `Command.UNLINK` 做
  `comodel.browse(command[1])` → 字符串被**逐字符**当成 id → `DELETE … WHERE id IN ('d','a','t',…)`
  → `invalid input syntax for type integer: "d"`。**正确写法**：`list.delete(record)`
  （`static_list.js` 的 `delete()` = `[DELETE, record.resId || record._virtualId]`，未保存行会撤销待发
  `CREATE`、不下发命令）；编辑字段用 `record.update(...)`。`record.id` 只能用于前端 key / 比较
  （`product_reference` 的同名教训见其 `onDelete()` 注释；`product_image` L1 约束 13 是硬不变量）。
...
3. 模块 `CHANGELOG.md`：变更 / 影响 / 文档同步三段式，注明日期；**修复 / 优化类版本**另加「优化目标」（变更之前）与「优化前后对比」（变更之后）两节，说明「为什么改、改前什么样」。条目正文（`##` 标题之下）固定加一行抬头
   `> 修订日期：YYYY-MM-DD ｜ 类型：<修复/功能/文档>（+x/+y/+z）｜ 影响文件：…`，并在「变更」里列**修正位置**（文件 + 方法 / 代码位置）与**涉及范围**（改了什么、没改什么、是否要迁移脚本）——让后来者不读代码也能回答「改在哪、为什么改」（写法参考 `product_variant/CHANGELOG.md` 与 `product_image` → `[19.0.2.6.6]`）
...
| `web_list_refresh` | 19.0.1.0.0 | 所有列表 / 看板 / 卡片视图统一刷新按钮：`t-inherit` 扩展 `web.ControlPanel` 插按钮 + patch `ListController` / `KanbanController` 用 `useSubEnv({ viewRefresh })` 注册处理器（缺处理器即不渲染，表单视图无按钮）；刷新走**无参** `model.load()`（复用 `RelationalModel.config` 里的 domain / groupBy / orderBy / limit / offset / currentGroups），搜索词 / 过滤器 / 收藏夹 / 分组展开态 / 排序 / 页码全保留；**显式回传 domain 会把 offset 归零，禁止** | 已交付（T-043，2026-09-25）：本地 `task check` 通过且新模块 0 告警、`odoo:19.0` 核心源码已核对；**待目标环境验证**（前端改动需 `-u` + 强刷浏览器）|
