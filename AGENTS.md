...
- **源语言固定英文（`en_US`）**：Python / XML / JS / QWeb 模板里所有用户可见文本一律写英文，中文不写回源码；不提供 `en_US.po`（源语言即英文）。
- **译文目录**：简体中文 `i18n/zh_CN.po`。新增语言时按 `<lang>.po` 追加（如 `zh_TW.po`），文件无需登记进 `data`，Odoo 自动扫描 `i18n/`。
- **可翻译入口**：
  - 字段 `string` / `help`、selection 标签、`models.Constraint` 消息、视图 / 动作 / 服务端动作文本 → `.po` 的 `model:` / `model_terms:` 条目（升级模块时重新导入）。
  - Python 运行期文案用 `_()`；JS 用 `_t()`；OWL 模板内联文本与 `title` / `aria-label` 由 OWL 渲染时按术语翻译（对应 `.po` 的 `code:` 条目）。
- **禁止拼接句子**：占位符统一 `%(name)s` 命名形式（`_()` 与 `_t()` 都支持），禁止 `"..." % (a, b)` 或 JS `+` 拼接整句；模板中夹子元素的句子（如 `<kbd>Ctrl</kbd>+<kbd>V</kbd>`）拆成完整可译片段由 JS 侧 `_t()` 提供。
- **收尾动作**：改英文源文本 → 同步 `.po` 的 `msgid` / `msgstr` → 提升版本 → `-u` 升级 + 强刷浏览器，英文与中文各验一遍。
- **代码注释保持中文**，不为 i18n 改英文。
