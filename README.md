...
| [`product_reference`](product_reference/README.md) | 19.0.2.7.1 | 产品多参考号 + 可在列表 / 选产品时按参考号搜索 + 多变体产品的**产品母型号**（`base_reference`：单变体产品只写变体编号，转多变体时上移成母型号、可搜到）；`19.0.2.3.0` 起移除「参考号」页，原生 `default_code`（Reference）输入框移到产品名下方（模板 + 变体表单，标签 `Ref.`），输入框内右端「+」以弹窗管理额外参考号，存在额外参考号时悬停徽标显示清单 tooltip；`19.0.2.4.0` 起多变体产品参考号不共用（各变体维护自己的一份，产品表单隐藏 `Ref.` 与「+」入口，该位置改显示母型号） | `19.0.2.2.0` 已交付并验证（T-007 / T-008；T-006 i18n 随 `19.0.2.0.0` 改名同步完成）；T-011（2026-09-08，落地 `19.0.2.5.0`）已完成并**目标环境验收通过**：移除「参考号」页 / Reference 输入框移到产品名下方（标签 `Ref.`）/ 输入框内「+」弹窗管理额外参考号 / 徽标悬停显示清单 tooltip / 多变体参考号不共用（各变体一份）/ 变体参考号可在产品列表搜索；验收记录见模块 `CHANGELOG.md` →「验收记录（T-011）」；`19.0.2.5.3`（T-013，2026-09-09）补应用列表（Apps）中文名 / 摘要 / 描述 + 分类「产品」译文，目标环境已验收通过；`19.0.2.6.0` / `19.0.2.7.0`（T-033，2026-09-22）新增母型号 `base_reference`：**单变体产品只写变体编号、转多变体时上移成母型号**（不做「两处同值」的镜像）+ 搜索并入 + 卡片可显示；本地已验证（开发库升级清理旧镜像值 22 条、跨模块 45 项测试 0 失败、中文标签与 help 落库），**待目标环境验证**；T-011 与 T-013 均已验收 |
| `product_image` | 19.0.2.6.4 | 产品多图：产品 / 产品变体双入口（19.0.2.6.0 起，每个变体各自维护一组独立补充图）+ 原生主图独立 + 图库补充图 / 主图无删除入口/ 首张上传即主图 / 主图 2 倍 / 悬浮局部放大（540窗口+1080图片平移·左侧不足转下方/缩小·选框按比例·留白区白色）/ 点击预览（多图切换+右侧缩略图·关闭按钮暗色半透明·底部条默认透明悬浮淡入·图片初始避开上下条放大可覆盖全屏·任意大小可拖拽grab/grabbing·GPU 1:1顺滑·切图保留状态·无滚动条·缩略图未选中无边框）/ 右侧竖排缩略图（蓝色选中边框·编辑态无删除按钮·滚动不越界且与主图区顶底贴边·仅切换不写库）/ 图片管理弹窗（「+」打开·顶层overlay：上半大图（仅预览、无删除按钮）+平铺缩略图（每张含主图右上角×：删图库删记录·删主图自动提升图库首张·点缩略图只切弹窗大图·缩略图可拖排序(含主图·首位即主图·避让动画)·删除均先确认(含缩略图·主图提示提升)·批量删除(勾选模式+含缩略图清单确认)·大图固定尺寸缩略图占满余量超高滚动·选中主图名称行留空）·下半dropzone 点击/拖放/Ctrl+V（上传中缩略图+动画·粘贴后不自动关闭·上传不改页面大图）·header 最右侧正方形×关闭按钮 hover 变红） | 已交付，目标环境已验证（T-005，19.0.2.4.2，含 19.0.2.2.10~4.2：拖动排序（含主图·首位即主图）/ 删除确认 / 批量删除 / 选中主图名称行留空 / 确认框按钮顺序 / 关闭按钮贴边；T-006 i18n 在 19.0.2.5.0 完成）；坑点与风险提示见模块 `AGENTS.md` →「会话修改总结与风险提示」/「开发复盘与关键经验（T-005）」（原名 `product_multi_image`）；T-010 变体多图（19.0.2.6.1）已完成，目标环境验收通过（2026-09-08，含图库图片占位回归修复与复验；验收记录见模块 `CHANGELOG.md` →「交付记录（T-010）」）；19.0.2.6.2（2026-09-08）修复变体上传报「双归属」Validation Error（变体 action context `default_product_tmpl_id` 污染图库子行创建，见模块 `CHANGELOG.md` → `[19.0.2.6.2]`），待目标环境复验；`19.0.2.6.4`（T-013，2026-09-09）补应用列表（Apps）中文名 / 摘要 / 描述 + 分类「产品」译文，目标环境已验收通过 |
| [`product_dimension`](product_dimension/README.md) | 19.0.4.1.0 | 为产品 Logistics 组增加物流尺寸（尺寸单位 cm / m + 长宽高，**存放在变体上**、每条变体各持一份，尺寸变化自动同步原生 Volume）；`19.0.2.0.0`（T-021）由 `product_packing` 改名而来，**移除纸箱与包装**字段，并把尺寸从模板下沉到变体 | `product_packing` 时期（T-009 / T-013）目标环境已验证；`19.0.2.0.1`（T-021，2026-09-22）改名 + 尺寸下沉 + 移除纸箱：本地已验证（旧数据搬运实测、**17 项**自动化测试、三表单合成 arch 实测、前端资源已确认进 `web.assets_backend`；`19.0.2.0.1` 修复 i18n 静默失效并打磨中文，字段 / 视图术语 / 应用列表已按数据库核对；`19.0.3.0.0` 补齐「变体快速编辑表单」挂载点、明确与原生 Volume 同进同退、`19.0.3.1.0` 表单填完尺寸立刻算出体积、`19.0.4.0.0` 体积改由前端算（后端只兜底、不再返回体积）、`19.0.4.1.0` 修 cm 小体积被显示成 0（前端取整参数 + Volume 精度提到 6 位）），**待目标环境验证** |
| [`product_card_view`](product_card_view/README.md) | 19.0.2.1.2 | 产品列表卡片视图（瀑布流）：为官方 Products 视图切换器新增 **Card** 按钮（库存 / 销售 / 采购入口），卡片顶部多图轮播（模板 / 变体两层图源不叠加）+ title / reference / on hand + 多变体按钮切换（一属性一行，禁用不存在组合），后端每页一次批量 `/product_card/payload`；卡片编号取「变体 `default_code` → 产品母型号 `base_reference` → 模板 `default_code`」；官方 list / kanban / form 本身不改；`product_image` / `product_reference` / `sale` / `purchase` 为可选依赖 | 已交付（T-012，2026-09-09）：Card 入口已确认；卡片内容 / 多图 / 变体 / 双语 / 权限待复验；`19.0.2.0.6`（2026-09-22）让销售 / 采购 / 发票入口也注入 Card，并把**所有产品列表的默认视图定为 Card**（按模型扫描 + 记录带 `sequence`，开发库核对 7 个动作的 `action.views`）；`19.0.2.0.6`（T-025）切换器 Card 按钮名走 `_t()` getter 并中文化，同时补齐 5 条此前失效的 JS 译文；`19.0.2.0.6` 注入改为读取时计算（覆盖 `_compute_views()`），修掉「全新安装时 sale / purchase 尚未安装导致漏注入」（`task init -- --fresh` 已实测），界面待复验（清单见模块 `README.md`，技术设计与踩坑见模块 `AGENTS.md`）；`19.0.2.0.7`（2026-09-22）修复切换 filter / group by 后卡片空白、过宽、无间隙（取数覆盖分组 + payload 填充时机 + 分组布局，见模块 `AGENTS.md` → L2 P6）；`19.0.2.0.6`（T-013，2026-09-09）补应用列表（Apps）中文名 / 摘要 / 描述 + 分类「产品」译文，目标环境已验收通过（Card 其余项仍待复验）；`19.0.2.1.0`（T-033，2026-09-22）卡片编号优先读产品母型号 `base_reference`（`product_reference` 可选、按字段存在性软适配），并修掉应用详情描述的 RST 列表渲染；`19.0.2.1.1` 收紧为**只有多变体产品**才读母型号（单变体产品的编号真身在变体上，残留值不得盖掉它）：本地已验证（升级无告警、描述渲染 8 个列表项、元数据一致性校验通过），卡片显示待目标环境验证 |
| [`product_variant_conversion`](product_variant_conversion/README.md) | 19.0.5.2.1 | 给产品**追加属性 / 取值**而不丢变体，**入口不是按钮**：用户照常在原生「属性与变体」页改属性、点保存，保存这一步自己检测这次变更（服务端用「只写配置、不碰变体」的试写分析）；会新增变体就弹窗逐组合确认「由哪条既有变体继续承载」，**确认之前保存不继续**，确认后属性变更与归属同一次写库；会丢既有变体（删取值 / 删属性）则拒绝保存，绝不静默归档或删除。每条既有 `product.product` 记录（id 不变）都保留自己的库存、库存移动明细、销售 / 采购 / 发票行、供应商价格与补货规则；转换留下**转换台账 + 变体谱系**，变体上有可搜索的「所属转换 / 来源变体」，下单、查询、管理都能追溯来源与归属。`19.0.5.1.0` 起：单变体产品的内部参考号**上移**为产品母型号（留在产品上、原变体编号清空，见 `product_reference`），`19.0.5.2.0` 起改为覆盖式（以变体编号为准）、变体没有编号时什么都不做；本来就是多变体的产品一个编号都不动。**本地已验证两种配置（只装 `product` / 加装 `product_reference`）各 45 项用例 0 failed / 0 error；加装 `product_dimension`、加装 `stock` + `sale_management` 两种组合与目标环境待验证** | 已交付（T-015，2026-09-21；`19.0.3.0.0` 起入口改为保存拦截 + 归属弹窗并移除按钮 / 向导；技术名在交付前由 `product_variant_convert` 定名调整为 `product_variant_conversion`，见模块 `CHANGELOG.md`）：归属怎么指定、被拒绝的情况、来源怎么追溯、已有业务数据怎么处理见模块 `README.md`；Odoo 变体生成机制、保存前钩子、i18n 与字段命名坑见模块 `AGENTS.md` → L1 / L2（P1~P5；P5 是场景覆盖矩阵、价格 / 库存同步规则、9 条已识别缺口与扩展点） |
...
`product_reference`（参考号 / 母型号）、`product_variant_conversion`（属性转换）、
...
|----------|--------------------|------------------------------|---------------------|
| 三个都装 | 母型号由转换上移进来、卡片显示它 | 上移编号、交接产品级共享参考号 | 卡片编号显示母型号（多变体产品） |
| 缺 `product_variant_conversion` | 母型号**不会自动产生**：多变体产品需在产品表单人工补录；单变体编号仍只落在变体上 | — | 卡片编号回退 `default_code`（多变体产品为空 → 显示 `—`） |
| 缺 `product_reference` | — | 编号留在原变体上、共享参考号留在产品级（**数据都不丢**，只是没有母型号这一层与变体的参考号入口） | 卡片编号回退 `default_code` |
...
3. **`product_variant_conversion` 会改动核心写行为**（拦截改属性的 `write()` —— 这是它的功能本身）：
   其它模块的程序化代码 / 测试若要产生多变体产品，请把属性行放进 **`create()`**（`create` 不拦截）。
...
| `product_reference` | 19.0.2.7.1 | 产品参考号 | 库存 / **产品** | `19.0.2.5.3` 及以前已验收；`19.0.2.6.0` / `19.0.2.7.0` / `19.0.2.7.1` 母型号 `base_reference`（单变体只写变体、转多变体时上移）+ 契约测试与解耦边界：本地已验证、**待目标环境验证** |
| `product_image` | 19.0.2.6.4 | 产品图片 | 库存 / **产品** | 已验收 |
| `product_dimension` | 19.0.4.1.0 | 产品尺寸 | 库存 / **产品** | 待目标环境验证 |
| `product_card_view` | 19.0.2.1.2 | 产品卡片视图 | 库存 / **产品** | Card 入口已核对，卡片内容待复验；`19.0.2.0.7` 修复切换 filter / group by 后卡片空白、过宽、无间隙；`19.0.2.1.0` 卡片编号读产品母型号、`19.0.2.1.1` 收紧为仅多变体产品、`19.0.2.1.2` 探测收敛进适配层 + 4 项 payload 用例（待目标环境验证界面） |
| `web_multi_tabs` | 19.0.2.0.0 | 后台多标签页 | 生产力（官方） | 元数据已写入，**整包待验证** |
| `product_variant_conversion` | 19.0.5.2.1 | 产品变体转换 | 库存 / **产品** | 元数据已写入，**整包待验证**；`19.0.5.1.0` / `19.0.5.2.0` 单变体编号上移为产品母型号（覆盖式）、`19.0.5.2.1` 可选集成适配层 + 降级用例（本地四组环境测试通过） |
