...
| 仓库扩展 | `.dev/init.yaml` 的 `addons`（当前登记 8 个，`web_multi_tabs` 暂被注释；**留空 = 自动发现全部**，新模块不用登记），改完跑 `task init` |
...
| `product_reference` | 19.0.3.1.0 | 产品参考号 | 库存 / **产品** | `19.0.2.5.3` 及以前已验收；`19.0.2.6.0` ~ `19.0.3.0.0` 产品级编号 `base_reference`（叠加进原生 `default_code` 的 compute + 单变体两处同值 + 两层参考号都可见 + 存量回填）：本地已验证、**待目标环境验证**；`19.0.3.1.0` 型号一律大写（CSS 显示 + 提交时转一次，避免吃字），本地 15 项测试 0 failed、**待目标环境验证** |
...
| `product_variant` | 19.0.7.0.0 | 产品变体 | 库存 / **产品** | 元数据已写入，**整包待验证**；`19.0.7.0.0` 改名 + 映射表（显示名随之改为「产品变体」，po 已同步）；`19.0.6.0.0`（`T-039`）**支持「按需生成变体」的属性**：改属性只展开「立即」轴、按需轴按既有变体现带取值钉住，带按需属性的产品不做价格分离；建产品时带按需属性仍拦住（原生会建出 0 变体）；47 项测试本地 0 failed；`19.0.5.3.2` 修「一次加两个属性」时的 chatter `Expected singleton`；`19.0.5.3.0` 与 `product_reference` 零耦合、变体值原样保留 |
...
1. 目标环境升级（可一次带全部模块）：
   ...
   product_dimension,product_card_view,product_variant,sale_product_hover --stop-after-init
   ```

   > 上面是**仓库当前全部 9 个模块**；日常只升级改动到的模块即可。
