...
| [`product_variant_conversion`](product_variant_conversion/README.md) | 19.0.5.3.2 | 给产品**追加属性 / 取值**而不丢变体，**入口不是按钮**：用户照常在原生「属性与变体」页改属性、点保存，保存这一步自己检测这次变更（服务端用「只写配置、不碰变体」的试写分析）；会新增变体就弹窗逐组合确认归属，确认后属性变更与归属同一次写库；会丢既有变体则拒绝保存。每条既有 `product.product` 记录（id 不变）都保留自己的库存、单据、价格与补货规则；**变体上的值（含编号、参考号、图片）一律原样保留**（`19.0.5.3.0` 起与 `product_reference` 零耦合）；转换留下**台账 + 谱系**供追溯。仅依赖 `product`。**「按需生成变体」的属性不支持**（那种产品由 Odoo 按订单建变体）：改属性与**建产品**两处都会拦住，报错点名属性并给出可执行的出路 |
...
| `product_variant_conversion` | 19.0.5.3.2 | 产品变体转换 | 库存 / **产品** | 元数据已写入，**整包待验证**；`19.0.5.3.2` 修「一次加两个属性」时的 chatter `Expected singleton`（整单回滚）；`19.0.5.3.1` 按需生成属性的拦截补齐（报错点名属性 + 建产品时就拦住）；46 项测试本地 0 failed；`19.0.5.3.0` 与 `product_reference` 零耦合、变体值原样保留 |
