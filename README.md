...
| `product_image` | 19.0.2.6.6 | 产品图片 | 库存 / **产品** | 已验收；`19.0.2.6.5` 产品列表「Images」列改为主图缩略（本地 7 项测试 0 failed），**待目标环境界面复验**；`19.0.2.6.6` 修复「未保存的新图库行删除 / 换主图 → 同图重复、删不掉、保存报 SQL 类型错」（修正位置：`static/src/js/product_image_gallery.js` 三处删除，均改走官方 `StaticList.delete(record)`），**待目标环境复验** |
