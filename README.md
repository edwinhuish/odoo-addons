...
| `product_image` | 19.0.2.0.3 | 产品多图：原生主图独立 + 图库补充图（主图作为浏览序列首张，图库按序其后）/ 主图 2 倍 / 悬浮放大 / 点击预览（放大缩小旋转）/ 右侧竖排缩略图（主图替换清空·图库删除·滚动·上传占位）/ 粘贴新增 | 已交付，**待目标环境验证**（原名 `product_multi_image`） |
