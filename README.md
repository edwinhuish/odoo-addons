...
| [`product_reference`](product_reference/README.md) | 19.0.3.1.1 | 产品多参考号 + 可在列表 / 选产品时按参考号搜索 + **产品级编号** `base_reference`（`19.0.3.0.0` 起叠加进原生 `default_code` 的 compute：产品表单 / 列表 / 搜索 / 卡片都直接可见；单变体产品与变体编号两处同值、多变体产品各管一层；产品级与变体级参考号都可见）；`19.0.2.3.0` 起移除「参考号」页，原生 `default_code`（Reference）输入框移到产品名下方（标签 `Ref.`），输入框内右端「+」以弹窗管理额外参考号 | 已交付，目标环境已验证（T-002 / T-011，`19.0.2.5.0`，2026-09-08）；`19.0.3.0.0`（T-035 / T-037，2026-09-23）产品级编号叠加进原生 `default_code` + 编号口径定稿：本地 52 项测试 0 failed、开发库升级实测回填 23 条单变体产品，**待目标环境验证**；存量多变体产品的产品编号需人工补录一次；`19.0.3.1.0`（2026-09-24）**型号一律大写**（CSS 显示 + 提交时转一次大写，避免逐键改写 `value` 的吃字，后端 `create` / `write` 兜底覆盖导入 / API）：本地 15 项测试 0 failed，**待目标环境验证**（前端需强刷浏览器）；`19.0.3.1.1`（2026-09-25）额外参考号弹窗 `Done` 右对齐 + 点弹窗外不再关闭，**待目标环境验证**（强刷浏览器） |
...
| `product_reference` | 19.0.3.1.1 | 产品参考号 | 库存 / **产品** | `19.0.2.5.3` 及以前已验收；`19.0.2.6.0` ~ `19.0.3.0.0` 产品级编号 `base_reference`（叠加进原生 `default_code` 的 compute + 单变体两处同值 + 两层参考号都可见 + 存量回填）：本地已验证、**待目标环境验证**；`19.0.3.1.0` 型号一律大写（CSS 显示 + 提交时转一次，避免吃字），本地 15 项测试 0 failed、**待目标环境验证** |
