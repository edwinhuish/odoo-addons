...
| `product_variant_conversion` | 19.0.5.3.2 | 产品变体转换 | 库存 / **产品** | 元数据已写入，**整包待验证**；`19.0.5.3.2` 修「一次加两个属性」时的 chatter `Expected singleton`（整单回滚）；`19.0.5.3.1` 按需生成属性的拦截补齐（报错点名属性 + 建产品时就拦住）；45 项测试本地 0 failed；`19.0.5.3.0` 与 `product_reference` 零耦合、变体值原样保留 |
