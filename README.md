...
| [`product_reference`](product_reference/README.md) | 19.0.2.1.0 | 产品多参考号 + 可在列表 / 选产品时按参考号搜索；第一行为 Odoo `default_code`（Reference）镜像，双向同步且不可删除 | 已交付，目标环境已验证；`19.0.2.0.0` 由 `product_model` 改名，`19.0.2.1.0` 新增内部参考行镜像；升级后待验证 |
