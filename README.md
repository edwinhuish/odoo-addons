...
| `product_variant_conversion` | 19.0.6.0.0 | 产品变体转换 | 库存 / **产品** | 元数据已写入，**整包待验证**；`19.0.6.0.0`（`T-039`）**支持「按需生成变体」的属性**：改属性只展开「立即」轴、按需轴按既有变体现带取值钉住（其它取值等订单创建），带按需属性的产品不做价格分离；建产品时带按需属性仍拦住（原生会建出 0 变体）；47 项测试本地 0 failed；`19.0.5.3.2` 修「一次加两个属性」时的 chatter `Expected singleton`；`19.0.5.3.1` 按需属性的建产品拦截与点名文案；`19.0.5.3.0` 与 `product_reference` 零耦合、变体值原样保留 |
