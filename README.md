...
| [`sale_product_hover`](sale_product_hover/README.md) | 19.0.1.0.1 | 报价单 / 销售订单订单行悬停展示产品详情浮层：产品图片、名称、型号、销售描述、产品售价、本单单价、可用库存；每页一次批量 payload + 浏览器缓存，悬停不发请求；仅作用于 `sale.order.line` 列表，不新增字段 / 权限 / 视图 | 已开发（T-014，2026-09-12，`19.0.1.0.1`），**待目标环境验证**；`19.0.1.0.1`（2026-09-13）修复悬停不触发（document 级事件委托 + 不抢焦点 + 接口兜底，见模块 `CHANGELOG.md`）；验证清单见模块 `README.md` |
...
| `sale_product_hover` | 19.0.1.0.1 | 订单行产品悬浮卡 | 销售（官方） | 元数据已写入，待验证 |
...
12. **T-014 订单行产品悬浮卡**（`sale_product_hover`）— 已开发（`19.0.1.0.1`，2026-09-13），**待目标环境验证**：报价单 / 销售订单订单行悬停弹出产品详情浮层（图片 / 名称 / 型号 / 描述 / 售价 / 本单单价 / 可用库存）；每页一次批量 payload + 浏览器缓存，悬停不发请求；不改官方视图、不新增字段与权限；`19.0.1.0.1` 修复悬停不触发（事件改 document 级委托、浮层不抢焦点、接口字段过滤与异常兜底），排障步骤见模块 `README.md`
