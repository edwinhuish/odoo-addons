...
| [`sale_product_hover`](sale_product_hover/README.md) | 19.0.1.1.0 | 报价单 / 销售订单订单行悬停（触屏为长按）展示产品详情浮层：产品图片、名称、型号、规格（变体属性）、销售描述、订单数量、单价、产品售价、可用库存；每页一次批量 payload + 浏览器缓存，悬停不发请求；浮层随视口收窄并在空间不足时自动换侧；仅作用于 `sale.order.line` 列表，不新增字段 / 权限 / 视图 | 已开发（T-014，2026-09-12），**待目标环境验证**；`19.0.1.1.0`（2026-09-14）重做触发链路修复订单页悬停不弹浮层（document 捕获阶段委托 + 以行 `data-id` 反查归属，去掉事件双绑定）、补齐规格 / 数量 / 单价展示、新增触屏长按与响应式（见模块 `CHANGELOG.md`）；验证清单见模块 `README.md` |
...
| `sale_product_hover` | 19.0.1.1.0 | 订单行产品悬浮卡 | 销售（官方） | 元数据已写入，待验证 |
...
12. **T-014 订单行产品悬浮卡**（`sale_product_hover`）— 已开发（`19.0.1.1.0`，2026-09-14），**待目标环境验证**：报价单 / 销售订单订单行悬停（触屏长按）弹出产品详情浮层（图片 / 名称 / 型号 / 规格 / 描述 / 订单数量 / 单价 / 产品售价 / 可用库存）；每页一次批量 payload + 浏览器缓存，悬停不发请求；浮层自适应视口并自动换侧；不改官方视图、不新增字段与权限；`19.0.1.0.1` / `19.0.1.0.2` 曾尝试修复悬停不触发，`19.0.1.1.0` 重做触发链路（document 捕获阶段委托 + 以行 `data-id` 反查归属、去掉事件双绑定）并补齐字段与触屏支持，排障步骤见模块 `README.md`
