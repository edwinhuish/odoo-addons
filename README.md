...
| [`product_reference`](product_reference/README.md) | 19.0.2.3.0 | 产品多参考号 + 可在列表 / 选产品时按参考号搜索；`19.0.2.3.0` 起移除「参考号」页，原生 `default_code`（Reference）输入框移到产品名下方（模板 + 变体表单），右侧「+」以弹窗管理额外参考号，存在额外参考号时悬停徽标显示清单 tooltip | `19.0.2.2.0` 已交付并验证（T-007 / T-008；T-006 i18n 随 `19.0.2.0.0` 改名同步完成）；`19.0.2.3.0`（T-011，2026-09-08）已交付，**待目标环境验收**，待验证清单见模块 `CHANGELOG.md` / `README.md` |
