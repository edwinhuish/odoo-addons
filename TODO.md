...
| ✅ | `product_image` | 19.0.2.2.7 | 产品多图（T-004）：原生主图独立 + 图库补充图 / 删除主图自动提升（无替换按钮）/ 首张上传即主图 / 主图 2 倍 / 悬浮局部放大（540窗口+1080图片平移·左侧不足转下方/缩小·选框按比例·留白区白色）/ 点击预览（多图切换+右侧缩略图·关闭按钮暗色半透明·底部条默认透明悬浮淡入·图片初始避开上下条放大可覆盖全屏·任意大小可拖拽grab/grabbing·GPU 1:1顺滑·切图保留状态·无滚动条·缩略图未选中无边框）/ 右侧竖排缩略图（蓝色选中outline·删除提升·图库删除·滚动·无tooltip）/ 新增图片弹窗（顶层overlay·点击·拖放·Ctrl+V 粘贴后关闭），目标环境已验证 |
...
- 完成日期：2026-09-04（目标环境验证通过）
- 落地版本：`19.0.2.2.7`（原名 `product_multi_image`，于 19.0.2.0.0 改名；19.0.1.0.0 为初版左右切换交互）
...
- 验收标准：
  - [x] 主图作为浏览序列首张（有主图时，无角标），图库图片按序跟在后面
  - [x] 首张即主图：空产品上传第一张图成为主图；已有主图时上传追加图库（不影响主图）
  - [x] 删除主图提升：删主图后图库首张自动提升为主图（【图A(主),B,C,D】→删A→【图B(主),C,D】）；图库为空则清空
  - [x] 选中蓝色边框：点击缩略图切换后，选中项带一圈蓝色边框（outline，未选中无任何边框）
  - [x] 新增图片弹窗：点击末端「+」弹出弹窗，弹窗内可点击选文件 / 拖放图片 / Ctrl+V 粘贴上传
  - [x] Ctrl+V 修复：弹窗打开时直接 Ctrl+V 即上传（无需先获焦），粘贴上传后弹窗自动关闭
  - [x] 选中边框四周完整：选中缩略图四周都显示蓝色边框（outline 不被裁切）
  - [x] 产品表单主图 180×180 显示，无上一张/下一张按钮、无序号
  - [x] 悬浮局部放大：540×540 窗口 + 1080×1080 图平移；左侧空间不足转下方，下方仍不足按比例缩小窗口；鼠标位置与放大区域精确对应（按图片内容算，不受留白影响）
  - [x] 蓝色指示方块：悬浮放大时主图上鼠标处显示蓝色半透明方块，边长按 (窗口/图片) 比例动态调整，标示实际被放大的区域
  - [x] 点击主图弹出全屏预览：放大/缩小/重置/旋转可用（按钮 + 滚轮 + 键盘），任意大小可拖拽（grab/grabbing），切换图片保留各图状态，无 Y 轴滚动条
  - [x] 主图项 × 按钮删除主图（提升或清空，无替换按钮）；图库项 × 按钮删除该图库记录；删除按钮位于缩略图右上角内侧、完整显示不被裁切
  - [x] 缩略图超出主图高度时出现上下滚动按钮，点击可滚动
  - [x] 只读态：无「+」占位符 / 删除 / 替换按钮；悬浮放大与点击预览仍可用
  - [x] 主图独立：图库增删改不影响产品主图；列表/看板展示原生主图
  - [x] 产品列表/看板展示主图
  - [x] 报价单、销售订单行可引用产品图片（此项可后置）
- 验证状态：目标环境已验证上述全部验收标准通过
