...
- **T-044 修复「新建产品上传 2 张图后把第 2 张拖到首位（换主图）→ 界面变 3 张（同图重复、多出的删不掉），保存报 `DELETE FROM product_image_gallery WHERE id IN ('d','a','t',…)`」** ｜ `product_image` ｜ P0
  ...
  - 做法：`onGalleryRemove()` / `onMainRemove()` / `_writeOrderWithNewMain()`（**生产报错的触发路径**）三处删除统一改走官方 `StaticList.delete(record)`（`[DELETE, record.resId || record._virtualId]`）——已保存行 → `(2, resId)`；未保存的新行 → `DELETE` 被识别为「该 id 上还有待发的 `CREATE`」→ **只撤销新建、不下发命令**。顺带删掉不再使用的 `x2ManyCommands` 导入、更正 `_itemKey` / `_gidFromKey` 注释，并把不变量写进模块 `AGENTS.md` → L1 约束 13
  - 验收记录：模块 [`CHANGELOG.md`](product_image/CHANGELOG.md) → `[19.0.2.6.6]` 与 [`AGENTS.md`](product_image/AGENTS.md) →「回归修复（19.0.2.6.6）」；`node --check product_image/static/src/js/product_image_gallery.js` 通过；`task check` 通过（未新增结构性问题）。根因结论取自 `odoo:19.0` 镜像内源码：前端 `model/relational_model/static_list.js`（`delete()`、`_applyCommands()` 的 `DELETE` / `UNLINK` 分支）、`datapoint.js`（`this.id = getId("datapoint")`），后端 `odoo/orm/fields_relational.py::One2many.write_real`（`Command.UNLINK` → `browse(command[1])`）
