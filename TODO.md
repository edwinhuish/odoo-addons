...
- [ ] 🚧 T-009 ｜ product_packing ｜ P1 ｜ 为产品 Inventory 标签页增加外贸纸箱字段（装箱数、长宽高、毛重、净重、自动 CBM），尺寸单位厘米 / 米可配置
  ...
  - 验收标准
    - [ ] 模块在目标环境安装成功，无报错
    - [ ] 消费品（`type = consu`）产品表单的 Inventory 标签页可见「纸箱与包装」组
    - [ ] 装箱数、长宽高、尺寸单位、毛重、净重可输入并保存
  ...
  - 备注
    - 落地版本 `19.0.1.0.0`（新模块，功能新增，第三位为 1）
