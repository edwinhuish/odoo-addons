...
- [ ] T-014 ｜ sale_product_hover ｜ P1 ｜ 报价单 / 销售订单产品行鼠标悬浮显示产品详情浮层
  - 需求：在报价单与销售订单的订单行列表上，鼠标悬停某一行时弹出浮层，展示该行产品的**名称、规格、图片**等关键信息（另含型号、描述、产品售价、可用库存）；移开鼠标后浮层消失（或延迟关闭）；展示效果清晰、响应及时、在不同设备上均可正常使用。**数量与单价按后续需求移除**（`19.0.1.5.0`：浮层只做"产品详情"，本单数据在订单行上本来就看得见）。
  - **设计约束**（动工前必读，不得自行放宽）：
  ...
  - **验收标准**：
    - [ ] 报价单（`draft` / `sent`）与销售订单（`sale`）的订单行列表，悬停行后浮层显示产品图片 / 名称 / 规格（另含型号 / 描述 / 产品售价 / 可用库存），**不显示订单行的数量与单价**
