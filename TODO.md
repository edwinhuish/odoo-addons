...
| 🚧 | `product_image` | 19.0.2.0.3 | 产品多图（T-004）：原生主图独立 + 图库补充图（主图作为浏览序列首张，图库按序其后）/ 主图 2 倍 / 悬浮放大 / 点击预览（放大缩小旋转）/ 右侧竖排缩略图（主图替换清空·图库删除·滚动·上传占位）/ 粘贴新增，待目标环境验证 |
...
- 落地版本：`19.0.2.0.3`（原名 `product_multi_image`，于 19.0.2.0.0 改名；19.0.1.0.0 为初版左右切换交互）
- 功能：在原有图片位置（头像区域）直接支持多图浏览，不新增页签，保持界面简洁
  ...
  - **主图独立 + 展示序列（19.0.2.0.3，按用户需求）**：
    - 移除 `_sync_main_image_from_template`/`is_main`/`_get_main_image`/gallery 的 create-write-unlink override，主图与图库彻底解耦
    - 浏览序列 = [原生主图（若有）] + [图库图片按 sequence 升序]，主图永远作为第一张
    - 主图项缩略图带铅笔「替换主图」+ ×「清空主图」按钮（写/清空 image_1920 字段），图库项带 × 删除按钮
  - 产品列表新增「图片数」列；图库独立列表/表单/搜索视图与动作（已移除 is_main 引用）
  - 图片行 `ondelete='cascade'`，删除产品时无孤儿数据
- 验收标准：
  - [ ] 主图作为浏览序列首张（有主图时），图库图片按序跟在后面
  - [ ] 产品表单主图 180×180 显示，无上一张/下一张按钮、无序号
  - [ ] 鼠标悬浮主图弹出放大图（左侧优先，空间不足转下侧/右侧）
  - [ ] 点击主图弹出全屏预览：放大/缩小/重置/旋转可用（按钮 + 滚轮 + 键盘）
  - [ ] 主图项铅笔按钮替换主图、× 按钮清空主图；图库项 × 按钮删除该图库记录
  - [ ] 缩略图超出主图高度时出现上下滚动按钮，点击可滚动
  - [ ] 缩略图末端上传占位符可新增图库记录（一次多图，不影响主图）
  - [ ] 头像区域 Ctrl+V 粘贴图片新增图库记录
  - [ ] 只读态：无上传/删除/粘贴；悬浮放大与点击预览仍可用
