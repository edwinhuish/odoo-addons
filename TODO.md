...
- **T-042 模块改名 `product_variant` + 「属性 ↔ 变体」映射表（变体未映射就不许保存）** ｜ `product_variant` ｜ P1
  ...
  - 落地版本：`product_variant` `19.0.7.0.0`
  ...
  - 验收记录：模块 [`CHANGELOG.md`](product_variant/CHANGELOG.md) → `[19.0.7.0.0]`；55 项服务端测试 0 failed / 0 error（含新增 8 项映射测试）；dev 库原地改名三条 SQL + `-u product_variant` 升级无报错（台账 / 视图路径已更新）；`task check` 无失败项
