...
| 🚧 | `product_image` | 19.0.2.0.4 | 产品多图（T-004）：原生主图独立 + 图库补充图（主图作为浏览序列首张，图库按序其后）/ 删除主图自动提升图库首张为主图 / 首张上传即主图 / 主图 2 倍 / 悬浮放大 / 点击预览（放大缩小旋转）/ 右侧竖排缩略图（主图替换·删除提升·图库删除·滚动·上传占位）/ 粘贴新增，待目标环境验证 |
...
- 落地版本：`19.0.2.0.4`（原名 `product_multi_image`，于 19.0.2.0.0 改名；19.0.1.0.0 为初版左右切换交互）
- 功能：在原有图片位置（头像区域）直接支持多图浏览，不新增页签，保持界面简洁
  ...
  - **19.0.2.0.0 交互重做**（按用户需求）：
    ...
    - 缩略图竖排于主图右侧，溢出时上下滚动按钮；末端上传占位符（编辑态，新增图库记录）
    - 头像区域 Ctrl+V 粘贴剪贴板图片即新增图库记录（内建，无需 image_uploader）
    - 只读态：隐藏上传/删除/替换/粘贴；悬浮放大与点击预览仍可用
...
- 验收标准：
  ...
  - [ ] 缩略图末端上传占位符可新增图片（一次多图；主图为空时成为主图，否则追加图库）
  - [ ] 头像区域 Ctrl+V 粘贴图片新增（主图为空时成为主图，否则追加图库）
  - [ ] 只读态：无上传/删除/替换/粘贴；悬浮放大与点击预览仍可用
