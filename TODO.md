...
| 🚧 | `product_image` | 19.0.2.1.0 | 产品多图（T-004）：原生主图独立 + 图库补充图 / 删除主图自动提升（无替换按钮）/ 首张上传即主图 / 主图 2 倍 / 悬浮局部放大（放大镜跟随方块·方块在180容器内移动·到边缘即显示边缘）/ 点击预览（放大缩小旋转）/ 右侧竖排缩略图（蓝色选中边框·删除提升·图库删除·滚动·无tooltip）/ 新增图片弹窗（顶层overlay·点击·拖放·Ctrl+V 粘贴后关闭），待目标环境验证 |
...
- 落地版本：`19.0.2.1.0`（原名 `product_multi_image`，于 19.0.2.0.0 改名；19.0.1.0.0 为初版左右切换交互）
