...
- [ ] T-014 ｜ sale_product_hover ｜ P1 ｜ 报价单 / 销售订单产品行鼠标悬浮显示产品详情浮层
  - 需求：在报价单与销售订单的订单行列表上，鼠标悬停某一行时弹出浮层，展示该行产品的**名称、规格、数量、单价、图片**等关键信息（另含型号、描述、产品售价、可用库存）；移开鼠标后浮层消失（或延迟关闭）；展示效果清晰、响应及时、在不同设备上均可正常使用。
  - **设计约束**（动工前必读，不得自行放宽）：
  ...
  - **验收标准**：
    - [ ] 报价单（`draft` / `sent`）与销售订单（`sale`）的订单行列表，悬停行后浮层显示产品图片 / 名称 / 规格 / 数量 / 单价（另含型号 / 描述 / 产品售价 / 可用库存）
    - [ ] 含变体产品显示规格（如 `颜色: 红, 尺寸: L`），无变体产品不显示规格行；数量带计量单位
    ...
    - [ ] **新增（未保存）的产品行也能悬停预览**（数据直接取自产品，不依赖订单落库 / 服务端升级）；改数量 / 单价后浮层内容同步更新；未选产品时不弹
