...
| 🚧 | `product_model` | 19.0.1.0.0 | 产品多型号 + 可搜索（T-002），代码已交付待环境验证 |
...
- [x] 型号用独立明细模型 + `One2many` 挂在 `product.template` 上，禁止用逗号分隔塞进单个 `Char`
- [x] 搜索必须在数据库层面可用（可 `store` 的冗余字段 + 索引，或 `search` 方法 / `any` 域），禁止 Python 侧全表过滤
- [x] 型号在同一产品内不允许重复；不同产品间允许同型号，重复时需明确提示归属
- [x] 搜索命中型号时，结果显示「产品名（命中型号：xxx）」，便于区分
