...
- [ ] T-014 ｜ sale_product_hover ｜ P1 ｜ 报价单 / 销售订单产品行鼠标悬浮显示产品详情浮层
  - 需求：在报价单与销售订单的订单行列表上，鼠标悬停某一行时弹出浮层，展示该行产品的**名称、规格、图片**等关键信息（另含型号、描述、可用库存）；移开鼠标后浮层消失（或延迟关闭）；展示效果清晰、响应及时、在不同设备上均可正常使用。**价格一律不展示**：数量与单价已移除（`19.0.1.5.0`），产品售价也已移除（`19.0.1.6.0`）——浮层只做"产品详情"，价格信息在订单行与产品表单上本来就看得见。
  - **设计约束**（动工前必读，不得自行放宽）：
    1. **不改官方视图与核心源码**：不新增字段、不加隐藏列、不改 `sale` 的视图 / 动作；只 `_inherit` 扩展与前端 patch。
    2. **只对 `sale.order.line` 列表生效**：其他模型（采购行、发票行等）的列表不得出现浮层。
    3. **不影响原有行点击与编辑**：浮层渲染在 overlay 容器，不改变表格 DOM；编辑态避让只针对**正在被内联编辑的已保存行**（未保存的新行不受限，否则新增产品无法预览——`Record.isInEdition` 对 `!resId` 恒为真）。唯一例外是触屏长按弹出浮层后吃掉紧随的那一次 `click`（一次性监听）。
    ...
    7. **跨设备**：触屏没有 hover，用长按弹出同一浮层；浮层宽度随视口收窄、空间不足自动换侧，不得溢出屏幕。
  - **验收标准**：
    - [ ] 报价单（`draft` / `sent`）与销售订单（`sale`）的订单行列表，悬停行后浮层显示产品图片 / 名称 / 规格（另含型号 / 描述 / 可用库存），**不显示任何价格**（无订单行的数量与单价，也无产品售价）
    - [ ] 含变体产品显示规格（如 `颜色: 红, 尺寸: L`），无变体产品不显示规格行；可用库存带计量单位
    - [ ] 鼠标移开后浮层消失；移入浮层内部不关闭（且停止跟随，便于阅读）；快速划过不弹出
    - [ ] 浮层跟随鼠标移动；右侧放不下时翻到光标左侧、下方放不下时上移贴边，不被屏幕裁切
    - [ ] 悬停订单行时不再弹出单元格的黑色原生 tooltip（其他行与其他元素不受影响）
    - [ ] 触屏设备（平板 / 手机）长按订单行弹出浮层；滚动列表不弹出；长按不会同时打开记录
    - [ ] 窄屏下浮层不溢出屏幕，空间不足时自动换侧
    - [ ] 悬停/移开不影响行的点击进入、编辑、勾选与删除等原操作
    - [ ] 采购订单行、发票行等非销售订单行列表不出现浮层
    - [ ] **新增（未保存）的产品行也能悬停预览**（数据直接取自产品，不依赖订单落库 / 服务端升级）；换成别的产品后浮层内容同步更新；未选产品时不弹
    - [ ] 新增行与已保存行的卡片内容 / 数字格式一致；产品无图片时显示占位图标
    - [ ] 服务类等**不跟踪库存**的产品：不显示「可用库存」行，且**不留空的分隔线**（明细表整块不渲染）
    - [ ] 英文（默认）与中文（安装 `zh_CN` 后）界面文案均正确
    - [ ] `-u sale_product_hover` 升级无报错，控制台无 JS 报错
  - **文档沉淀**（均在 `sale_product_hover/`）：`README.md`（用法 / 字段 / 验证清单）、
    `CHANGELOG.md`（`19.0.1.0.0` → `19.0.1.6.0` 逐版本变更）、`AGENTS.md`（约束与踩坑档案）、
    **`IMPLEMENTATION.md`（实现说明：架构 / 数据流 / 核心要点 / 难点 / 17 条坑点速查）**、
    `RETROSPECTIVE.md`（过程复盘 / 方法论 / 自校验与冒烟脚本）
