...
### `web_multi_tabs` ｜ P2 ｜ 后台内部多标签页（适配 PWA standalone）

- 搁置日期：2026-09-04
- 原因：与进销存主流程无关，按需；模块来自仓库之外的本地备份目录，未迁入本仓库
- 决策出处：T-005 仓库规整子决策（已固化到根 `AGENTS.md` 第 8 节速查表、根 `README.md` 模块一览表，均标注「不迁移」）
- 复活条件：日后确需多标签页且无更优方案时，从仓库外备份目录迁移
