...
> **已归档（2026-09-24 文档复盘）**：本文件是环境**搭建过程**的记录（方案选型 / 实施步骤 / 当时的坑）。
> 日常操作请直接看 [`DEV_WORKFLOW.md`](../../DEV_WORKFLOW.md)；仍有效且需要长期遵守的几项
> （方案选型结论、镜像维护约束、后续优化方向）已并入它的第 1 节与第 8 节。
> 归档原因见 [`docs/README.md`](../README.md) →「四、本次归档的决策记录」。
