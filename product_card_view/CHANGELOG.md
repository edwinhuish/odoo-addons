...
## [19.0.2.1.4] - 2026-09-23（编号随选择切换：未选产品编号 / 选中变体编号）

> 修订日期：2026-09-23 ｜ 类型：优化（前端口径，+z）｜ 影响文件：
> `static/src/js/product_card_record.js` / `__manifest__.py` / `AGENTS.md` / `README.md`
> （模板 XML 与样式本期未改：版式维持 `title` → 「编号 + 在手」→ 已选组合行）

### 优化目标

`19.0.2.1.3` 把编号栏**固定成产品编号**，并把变体编号挪到已选组合行（`Blue / Large · FURN_6666`）。
使用中反馈这不合适：卡片选中变体后，用户想在第一眼的位置看到**这个变体的编号**，
而不是把编号藏在组合文本的尾巴上。本版改回「编号随选择切换」，并让已选组合行只管属性组合。

### 变更

1. `referenceText`：未选变体 = 产品编号（payload 模板层 `reference` = 原生
   `product.template.default_code`）；**选中变体 = `variants[].reference`（该变体的
   `product.product.default_code`）**，变体没填编号才回退产品编号；都没有显示 `—`。
2. `selectionText`：**移除**追加的变体编号，只拼属性值名（`Blue / Large`）。
3. 模板 / 样式未改（编号仍显示在 `o_product_card__meta` 行左侧、在手徽标右侧）。
   同日早些时候试过把编号移到产品名同一行（`o_product_card__heading`），已按要求回退、未发布。

### 优化前后对比

| 场景 | 优化前（`19.0.2.1.3`） | 优化后（`19.0.2.1.4`） |
|------|----------------------|----------------------|
| 未选变体 | 编号栏 = 产品编号 | 编号栏 = 产品编号（不变） |
| 选中变体（变体有编号） | 编号栏仍 = 产品编号；组合行 `Blue / Large · FURN_6666` | 编号栏 = `FURN_6666`；组合行 `Blue / Large` |
| 选中变体（变体无编号） | 组合行只有属性 | 编号栏回退产品编号，组合行只有属性 |
| 同一编号出现次数 | 1 次（编号栏） | 1 次（编号栏）—— 组合行不再重复 |

### 影响

- 编号栏的含义随选择切换：**卡片代表产品、点选变体后代表该变体**，与图片 / 在手数量的切换口径一致
- 变体编号不再需要用户在组合文本里找；组合行回到纯粹「属性组合」的表达
- payload、接口、后端 payload 字段均未改（两层 `reference` 早已下发，只改了前端取用与拼接）
- **前端改动 → 升级后必须强刷浏览器**（Ctrl+F5）
- 两个 getter 无自动化用例（无头环境断言不了渲染）→ 公布前人工验证「未选 / 选中」两态

### 文档

- 模块 `README.md`（信息区 / 已选组合行 / 默认口径 / 编号取值链 / payload 字段表两行 /
  异常情况与处理 / 后续维护）、`AGENTS.md`（L1 第 5 条重写为「编号随选择切换」+ 版式说明 +
  getter 无自动化用例的提醒 + 新增 L2 **P7**（编号口径三次定稿）与 **P8**（信息区 flex 陷阱，
  含「编号紧跟产品名」那次被回退的尝试））、本条目
- 根 `README.md` / `AGENTS.md` 版本行、`TODO.md`（`T-037`）同步

### 验证记录

| 项 | 结果 |
|----|------|
| `task test -- product_card_view` | 6 项 0 failed / 0 error（payload 未改，2 项按配置 skip）✓ |
| `task check` | 通过，未新增告警 ✓ |
| 卡片两态显示（未选产品编号 / 选中变体编号；组合行只有属性） | **目标环境待验证**（需强刷浏览器，中英各一遍） |
...
- 根 [`STAGE_REPORT_2026-09-22.md`](../docs/archive/STAGE_REPORT_2026-09-22.md)（已归档）：速览版本行、2.4 节、6.2 界面复验清单、6.3 文件地图、5.3 风险同步
