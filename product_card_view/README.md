...
> 模块技术目录：`product_card_view`（版本 `19.0.2.1.4`，T-012 已交付；**库存 / 销售 / 采购 / 发票入口均已注入 Card 且默认为 Card（开发库核对 `action.views`），卡片内容待目标环境复验**）。
>
> 修订日期：2026-09-23 ｜ 本次修订：`19.0.2.1.4`（**编号随选择切换**：未选变体=产品编号、
> 选中变体=该变体编号；已选组合行只显示属性组合）｜ 上一版修订：`19.0.2.1.3`（与
> `product_reference` 彻底解耦 —— 编号只读原生 `default_code`）。
...
- **图片轮播**：卡片顶部展示产品主图；多图时支持左右箭头、触屏 / 鼠标滑动翻页与右下角计数。
  ...
  - 模板层与变体层两套图**互不叠加**。
- **信息区**：产品名一行；其下是**编号 + 在手徽标**一行（`on hand`）；再其下是已选组合行。
  **编号随选择切换（`19.0.2.1.4` 起）**：未选变体时是**产品编号**（原生 `product.template.default_code`，
  单变体产品即那条变体的编号、多变体产品即产品编号）；选中变体后是**该变体的编号**
  （变体没填编号才回退产品编号）；都没有时显示 `—`。
- **已选组合行**：选中变体时**只显示所选属性**（如 `Blue / Large`）—— 变体编号不在这里，
  它由编号栏承担（`19.0.2.1.4` 起），同一编号不在卡片上出现两次。
...
本模块**不依赖**任何自研模块（`depends` 只有 `stock`）：卡片编号与图库都由可选模块补强，
缺席时对应那一层为空、其余照常。对每个可选模块的了解都收在**一处**（一个方法或一次判断），
对方改名 / 改字段只需改那处（约束见模块 `AGENTS.md` → L1 第 5 条）。
...
## 筛选 / 分组下的取数与布局（`19.0.2.0.7`）

> 本节记录一次修复的**目标 → 变更点 → 前后对比 → 预期效果**，便于后续查阅与追溯。
> 完整技术细节与踩坑见 `AGENTS.md` → L2 P6；逐版本记录见 `CHANGELOG.md`。

### 优化目标

在搜索框中**切换筛选（filter）或分组（group by）之后**，Card 视图必须与首屏表现一致：
卡片有主图（多图可轮播）、有产品信息（title / reference / on hand / 变体按钮）、
宽度适当、卡片之间有正常间隙；未分组与分组两种形态都要满足。

### 变更点

| # | 变更 | 文件 |
|---|------|------|
| 1 | 取数范围覆盖分组：新增 `collectCardRecords()`，未分组取 `list.records`、分组取 `list.groups[].list.records`（多级分组递归） | `static/src/js/product_card_model.js` |
| 2 | 取数时机：`fillProductCardPayload` 改为「请求前不清空、成功后整体替换」+ 请求序号防乱序 + 批次 key 防重复请求；渲染器新增 `useEffect` 比对本页 id，数据到位后显式 `render(true)` | `product_card_model.js` / `product_card_renderer.js` |
| 3 | 布局作用域：新增分组样式（宽度上限 + 卡片间距）；未分组卡片 `margin: 0` 且 `position: absolute` 只由 JS 写 inline；布局改在 `onPatched` / `onMounted` 触发并加容器 `ResizeObserver` | `static/src/scss/product_card.scss` / `product_card_renderer.js` |

### 优化前后对比

| 场景 | 修复前 | 修复后 |
|------|--------|--------|
| Group By 后卡片 | 无图、无产品信息（取数 id 为空导致整页空 payload） | 正常显示图片与产品信息 |
| 切换 filter / 翻页 / reload 后 | 卡片空白且**不恢复**（请求飞行期间读到已清空的 Map，非 reactive Map 又不再触发渲染） | 自动补拉并重渲染，表现与首屏一致 |
| 分组列里的卡片宽度 | 撑满列宽（官方 `width: 100%`） | `max-width: 22.5rem`，列内居中 |
| 分组列里的卡片间距 | 零间距（官方 `margin: 0 0 -1px`） | `0.75rem` |
| 未分组首帧 / 切筛选瞬间 | 卡片可能叠在一起（SCSS 里已 `absolute` 而 JS 未跑） | 退回正常流布局作兜底，JS 跑完再切换为瀑布流 |
| 未分组卡片间距 | 官方 margin 与瀑布流 `GAP` 叠加，间距不可控 | 间距完全由瀑布流 `GAP`（12px）决定 |

### 预期效果

- 首屏、切换筛选 / 分组、翻页、reload 四种操作下卡片表现一致：有图、有产品信息、宽度适当、间隙正常。
- 未分组仍为 JS 瀑布流（列高均衡、响应式列数）；分组视图卡片宽度 ≤ 22.5rem、间距 0.75rem。
- 后端接口 `/product_card/payload` 与数据库结构均无变化（分组时一次性提交所有分组的记录 id）；
  不新增用户可见文案，`i18n/zh_CN.po` 无需改动。
- 升级方式：`-u product_card_view` + 强刷浏览器（前端资源改动），无迁移脚本。
...
- **默认口径**：未选变体时显示模板名 / 产品编号 / 全部变体在手总和；
  非库存追踪产品（服务等）在手显示 `—`。选中变体后编号与在手都切为该变体。
- **编号取值链**（后端解析好再下发，前端只做回退）：
  - **未选变体**：模板层 `reference` = 原生 `product.template.default_code`
    —— 单变体产品=那条变体编号；多变体产品=产品编号
    （由 [`product_reference`](../product_reference/README.md) 算进该字段；未装它时为空）；
  - **选中变体**：变体层 `variants[].reference` = 变体 `default_code`
    （为空则回退模板 `reference`）；
  - 两者都没有 → 编号显示 `—`（说明产品 / 变体的 `Ref.` 还没填）。
...
| `reference` | `str` | 模板层（产品）编号，**未选变体时编号栏显示的就是它**：原生 `product.template.default_code` |
...
| `variants[].reference` | `str` | 该变体的编号（**选中变体后编号栏显示的就是它**）：变体 `default_code`，为空则回退模板 `reference` |
...
> 修订日期：2026-09-22（新增第 14、15 项与 TC-12、TC-13，对应 `19.0.2.0.7`）
...
> 第 14、15 项为 `19.0.2.0.7` 新增（切换筛选 / 分组），修复前必失败，需在目标环境复验。
...
| 14 | 切换筛选（filter）（19.0.2.0.7） | 在搜索框加 / 去筛选后卡片立即刷新：有图、有产品信息，宽度与间隙正常，不残留空白卡片 | 取数与重渲染链路已在开发库核对；界面待目标环境复验 |
| 15 | 分组（Group By）（19.0.2.0.7） | 按任意字段分组后，各分组列内卡片有图有信息、宽度 ≤ 22.5rem、卡片间距 0.75rem；再切回不分组仍是瀑布流 | 同上 |
...
| TC-12 切换筛选（19.0.2.0.7） | 在 Card 视图加一条筛选（如「有库存」）→ 观察卡片 → 再取消筛选 | 两次都立即显示图片与产品信息；卡片不空白、宽度与间隙不变 | 待验（修复前必失败） |
| TC-13 切换分组（19.0.2.0.7） | 分组（如按分类）→ 观察各列卡片 → 切回不分组 | 分组列内卡片有图有信息、宽度 ≤ 22.5rem、间距 0.75rem；切回后恢复瀑布流且无错位 | 待验（修复前必失败） |
...
5. **切换筛选 / 分组后的取数与布局（`19.0.2.0.7` 已修，待复验）**：修复前存在
   「Group By 后整页无图无信息」「切筛选后空白不恢复」「分组卡片过宽 / 零间距」三个问题；
   现已按 `AGENTS.md` L2 P6 的三条措施修复，需按 TC-12 / TC-13 在目标环境复验。
6. **`useEffect` 兜底重渲染的代价**：每次 patch 会比对一次本页 id（字符串 join），
   取到新数据时多一次深渲染；若后续列表页记录数显著增大（数百条以上），可考虑改为
   比较 id 列表长度 + 首尾 id 的轻量 key。
