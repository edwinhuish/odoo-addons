...
- 模板层：标题 / 编号 / 全部变体在手总量；编号只读**原生** `product.template.default_code`
  （多变体产品的产品编号由 `product_reference` 叠加进该字段的 compute，本模块无需知道它存在）；
...
class ProductTemplate(models.Model):
    ...
    def _get_product_card_view_payload(self):
        ...
        for template in templates:
            ...
            payload[tpl_id] = {
                "name": template.name,
                # 产品编号：只读原生 default_code（多变体产品的产品编号由
                # product_reference 叠加进该字段的 compute，本模块不需要知道它存在）
