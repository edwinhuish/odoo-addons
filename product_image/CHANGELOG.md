...
- 同步 `__manifest__.py`（版本 19.0.2.6.5）、模块 `README.md`（功能概述 / 视图 / 验证清单 / 执行流程 /
  异常情况与处理 / 后续维护）、`AGENTS.md`（L1 新增「列表图片列只能是主图」约束、当前版本、文件职责、
  接口与字段变更，并新增「开发复盘与关键经验（T-038）」含操作时间线 / 备选方案 / 坑点）、
  根 `README.md`（模块一览表版本与状态）、根 `TODO.md`（`T-038` 归档）、
  阶段报告 [`STAGE_REPORT_2026-09-24.md`](../docs/archive/STAGE_REPORT_2026-09-24.md)（已归档）（第 7 节）。
- 提交：`9748eeb fix(product_image): 产品列表 Images 列改显主图缩略`（2026-09-23 17:30:39 +0800，11 个文件）。
...
- **应用列表（Apps）中文化**：`i18n/zh_CN.po` 补充「应用列表元数据」译文，中文环境下应用卡片与详情页显示中文模块名 / 摘要 / 描述。
  - `model:ir.module.module,shortdesc:base.module_product_image` → 「产品图片」（并入已有 `Product Images` 条目，作为多引用，避免重复 `msgid`）
  - `model:ir.module.module,summary:base.module_product_image`：摘要整句译文
  - `model:ir.module.module,description:base.module_product_image`：`description` 整段译文（`translate=True` 整值翻译，`msgid` 与 `textwrap.dedent(manifest["description"])` 逐字符一致）
...
- `odoo -d <db> -u product_image --stop-after-init` 升级后，中文环境「应用」列表显示中文名称 / 摘要 / 描述与中文分类；英文环境不变。
...
- 同步 `__manifest__.py`（版本 19.0.2.6.3）、`README.md`（国际化节）、`AGENTS.md`（当前版本 + i18n 约束）、根 `README.md` / `AGENTS.md` / `TODO.md`。
...
- 验收结果：6 项验收标准全部通过（中英文各验一遍）
  - [x] `odoo -d <db> -u product_image --stop-after-init` 升级无报错
  - [x] 中文「应用」列表：卡片标题显示「产品图片」，摘要为中文
  - [x] 中文「应用」详情：描述整段为中文
  - [x] 中文「应用」左侧分类：显示「库存 / 产品」——父级 `Inventory` 沿用官方译文，自定义段 `Product` → 「产品」由本模块提供（并入已有条目，避免重复 `msgid`）
