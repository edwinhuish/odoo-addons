...
- `odoo -d <db> -u product_image --stop-after-init` + 强刷浏览器：英文界面下字段名 / 提示 / 报错 / 图片管理弹窗全部为英文。
- 启用 zh_CN 并切换语言后：字段标签、help、删除确认框、批量删除提示为中文；切换回英文恢复英文。
- 图片管理弹窗「已选 N 张」、缩略图删除按钮 aria-label、Ctrl+V 提示在两种语言下均完整且语序正确。
