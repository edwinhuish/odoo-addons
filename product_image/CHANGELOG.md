...
- **大图（弹窗左侧预览）不再提供删除按钮**：删除入口只保留在缩略图网格——仅非主图（图库）项右上角 × 可删除，主图项仍无删除按钮；顺带移除已无模板引用的 `deleteCurrent` 方法。
- **关闭按钮改为正方形、无文字、hover 背景变红**：位于 `.modal-header` 右侧，46×46、高度占满整个 header（header 上下无内边距），仅图标（fa-times），hover / 键盘聚焦时背景变红（#dc3545）；原「图标 + 文案」按钮形态移除。
...
- 交互变化：① 弹窗大图预览区对任何图片（含图库图）均不显示 ×；② 删除图库图只能点缩略图网格中该图右上角 ×；③ 上传（含 Ctrl+V 粘贴）不改变页面展示的主图 / 当前图；④ 右上角关闭按钮为正方形、无文字、hover 变红。
