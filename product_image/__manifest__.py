{
    ...
    "description": """
        ...
        - 点击缩略图列末端的「+」打开图片管理弹窗：上半部分大图预览（仅作预览，不提供删除按钮）+ 平铺缩略图网格（每张缩略图含主图右上角 ×：删除图库图删记录，删除主图自动提升图库首张为主图；点击缩略图只切换弹窗内大图、不影响页面主图），下半部分上传 dropzone（点击 / 拖放 / Ctrl+V 粘贴，上传中即时显示缩略图与上传动画，粘贴后不自动关闭，上传不改变页面当前显示的大图；内建，无需 web_image_paste 依赖）；关闭按钮位于 header 最右侧、正方形无文字（hover 变红）
