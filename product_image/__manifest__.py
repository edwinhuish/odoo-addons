{
    ...
    "description": """
        ...
        - 点击缩略图列末端的「+」打开图片管理弹窗：上半部分大图预览（仅作预览，不提供删除按钮）+ 平铺缩略图网格（删除入口只在缩略图——仅图库图右上角 × 可删除，主图项无删除；点击缩略图只切换弹窗内大图、不影响页面主图），下半部分上传 dropzone（点击 / 拖放 / Ctrl+V 粘贴，上传中即时显示缩略图与上传动画，粘贴后不自动关闭，上传不改变页面当前显示的大图；内建，无需 web_image_paste 依赖）；关闭按钮为 header 右上角正方形按钮（无文字，hover 变红）
