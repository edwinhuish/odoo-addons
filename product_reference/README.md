...
| 一块只在一个表单上渲染 | 产品级编辑器挂在产品模板表单标题区；变体完整表单（`product.product.form`）**继承**模板表单，所以那块要靠 `invisible="is_product_variant"`（模板侧为假 / 变体侧为真）在变体表单上隐藏，标题区只剩变体级编辑器（`19.0.3.1.2` 起）。去掉门控或在变体侧再挂一块，界面就会出现两个 `Ref.` 输入框 |
...
- **标题区每个表单只有一个 `Ref.` 输入框**（`19.0.3.1.2` 起）：变体完整表单继承产品模板表单，
  产品级那块靠 `invisible="is_product_variant"` 在那儿被隐藏，只留变体级编辑器；产品表单
  （含多变体产品）照常显示产品级编号与产品级参考号入口。挂载契约由
  `tests/test_view_mounting.py` 钉住
...
7. **多变体产品**：产品表单显示**产品级**编号（`Ref.` = `base_reference`）与产品级参考号
   （`19.0.3.0.0` 起不再按变体数隐藏）；打开具体变体（产品变体列表，或模板上的「变体」按钮），
   在该变体的产品名下方维护**它自己**的编号与参考号 —— 变体之间互不共用
...
- 弹窗的结构与类名沿用 Odoo 原生弹窗（`web.Dialog`），标题、关闭按钮、字体颜色全部
  走原生样式；**两处按使用习惯有意偏离原生**（`19.0.3.1.1`）：
  ① 底部 `Done` 按钮**靠右下角**（原生 19.0 的 footer 是左对齐）；
  ② **点弹窗外（灰色遮罩区域）不会关闭弹窗**，避免误点一下丢掉还没保存的录入。
- 关闭弹窗只有三个入口：**Esc** / 右上角 **×** / 底部 **Done**。
...
| `Done` 靠右下角（19.0.3.1.1） | 弹窗底部 `Done` 按钮在右下方，与表格右边缘对齐 | **待目标环境验证**（强刷浏览器） |
| 点弹窗外不关闭（19.0.3.1.1） | 点击弹窗外灰色遮罩区域，弹窗保持打开；Esc / × / `Done` 仍能关闭 | **待目标环境验证** |
| 变体详情只有一个 `Ref.` 框（19.0.3.1.2） | 从 Product Variants 点进变体详情，标题区只有**一个** `Ref.` 输入框（不再两个）；产品表单仍显示产品编号 + 产品级参考号；两张变体表单的「+」弹窗都只列**该变体**的参考号 | **本地通过**（合成 arch 实测 + `tests/test_view_mounting.py` 4 项）；目标环境界面待复验（强刷浏览器） |
