...
1. **没有属性时也给一行「空组合」**：`buildCombinationRows()` 原来在「一个有效轴都没有」时返回空表，
   导致把唯一属性删掉后面板显示「1 个既有变体未分配」、保存被拦。现在返回一个 `cells=[]` 的组合行，
   让既有变体可以挂靠。
2. **默认分配逻辑抽成纯函数**：新增 `applyDefaultAssignment()`，在 `refreshRows()` 里调用。
   空组合行会匹配所有既有变体；只有一条既有变体时自动挂上去、无需手动选，也不标成「未保存」。
   多条变体抢一个空组合时只取第一条，其余仍留在未分配里等用户决定。
3. 纯函数自测 `tests/js/variant_mapping_pure.mjs` 补了两个断言场景：
   - 删空属性后仍有 1 行空组合；
   - 唯一既有变体在删除属性后自动复用。

### 影响

- 行为修复：删除最后一个属性后，产品回到「无属性」状态，原来的那条变体会被自动保留，
  不会再提示未分配、也不会重建新变体。
- 纯前端改动；需 `-u` + 强刷浏览器。
