...
- 验收结果：6 项验收标准全部通过（中英文各验一遍）
  - [x] `odoo -d <db> -u sale_order_no --stop-after-init` 升级无报错
  - [x] 中文「应用」列表：卡片标题显示「订单编号」，摘要为中文
  - [x] 中文「应用」详情：描述整段为中文
  - [x] 中文「应用」左侧分类：显示「销售」——`Sales` 是官方分类，沿用 `base` 自带译文，本模块未重复翻译
...
- **国际化（i18n）**：源码用户可见文本全部改为英文（源语言 `en_US`），新增 `i18n/zh_CN.po` 提供简体中文翻译，支持中英双语、默认英文。
  - 模型：字段 `string` / `help`（`order_no` / `buyer_ref` / `buyer_order_idx`）、数据库唯一约束消息、流水号耗尽与编号占用的 `ValidationError`、批量补号的通知标题与消息。
  - `_()` 调用改用命名占位符 `%(xxx)s`。
  - 视图 / 动作：订单表单占位提示、列表与搜索视图的列标题、联系人「客户编号」标签与占位提示、服务端动作「生成订单编号」名称。
  - 报表 `print_report_name` 表达式与门户模板沿用 `order_no`（与语言无关，不受本次改动影响）。

### 影响

- **默认展示语言变为英文**：`-u` 升级后，未启用中文的数据库全部显示英文文案；报表与门户打印逻辑不变。
- 需要中文的库：设置 → 语言安装「简体中文 (zh_CN)」，`odoo -d <db> -u sale_order_no --stop-after-init` 升级并强刷浏览器。
- 联系人 `ref` 的格式校验错误与批量补号通知在中英文下均带出具体值。
- 代码注释仍为中文（不参与翻译）。

### 文档

- 同步 `__manifest__.py`（版本 19.0.1.8.0、name / summary / description 英文化）、`README.md`、`AGENTS.md`、根 `README.md`。
...
- 验收结果：两项验收标准全部通过
  - [x] 英文界面：订单编号字段标签 / help、联系人客户编号标签与校验报错、列表批量补号通知为英文
  - [x] 中文界面：上述内容显示为中文；切换语言后 PDF 文件名仍使用订单编号

### 交付记录（T-006，2026-09-07）

- **完成日期 / 落地版本**：2026-09-07，`19.0.1.8.0`。
- **需求**：四模块国际化之一——`sale_order_no` 源语言改为英文（`en_US`，默认展示英文），新增 `i18n/zh_CN.po` 简体中文译文，切换语言后界面随之更新；同时保证两种语言下 PDF 文件名与门户标题仍使用 `order_no`。
- **验收记录**：目标环境 `-u sale_order_no` 升级并强刷浏览器后，英文界面订单编号字段标签 / help、联系人客户编号标签与校验报错、列表批量补号通知均为英文；切换中文后上述内容显示中文；英文 / 中文两种语言下打印 PDF 文件名与门户标题均使用 `order_no`，未退化成 `object.name`。
- **异常与后续维护**：无。
