...
- **完成日期**：2026-09-17 ｜ **状态**：**已完成**（任务已归档，见根 [`TODO.md`](../TODO.md) →「已归档」→ T-014）
- **落地版本**：`19.0.1.6.0`（首版 `19.0.1.0.0`，2026-09-12 ~ 2026-09-16，共 15 个版本）
- **验收方式**：按 [`README.md`](README.md) →「验证清单」逐条执行（`-u sale_product_hover` 升级 + 重启进程 + **强刷浏览器**；中文环境另验一遍）；排障入口是控制台 `[sale_product_hover]` 开头的分段日志与 `self-check: assets X, server Y` 自证行
- **异常与维护**：17 条坑点档案（现象 / 原因 / 规避）见 [`IMPLEMENTATION.md`](IMPLEMENTATION.md) →「坑点档案」；约束与陷阱档案见 [`AGENTS.md`](AGENTS.md)；过程复盘、自校验脚本与冒烟清单见 [`RETROSPECTIVE.md`](RETROSPECTIVE.md)
- **遗留**：无阻断项；后续版本升级后按上述验证清单回归即可

---

## [19.0.1.6.0] - 2026-09-16（2026-09-17 验收完成）
...
- 新增 **`IMPLEMENTATION.md`（实现说明）**：架构分层与文件职责、两条取数路径、payload 字段口径、
  运行时序、16 条核心要点（做法 + 源码依据）、8 个实现 / 联调难点、**17 条坑点档案（现象 / 原因 / 规避）**
  （含本次新增的「空明细表分隔线」「前后端版本不同步」「po 孤儿术语」）与维护扩展清单。
- 模块 `README.md`（功能概述 / 核心设计 / payload 字段 / i18n / 验证清单 / **文档索引**）、`AGENTS.md`
  （当前版本、浮层内容约束、文件职责、文档同步清单）、`RETROSPECTIVE.md`（版本一览与冒烟清单、文档定位）、
  `__manifest__.py`（`19.0.1.6.0` + `description`），根 `README.md` / `AGENTS.md` / `TODO.md` 同步。
- 顺手修正几处随需求变更而**过时的源码注释**：JS 里「版本三处同步」→ 两处、新行取数上下文注释
  「数量 / 单价一变签名就变」→ 产品 id 变（价格字段已下线）。
