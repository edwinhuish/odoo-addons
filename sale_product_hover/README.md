...
- **自适应**：浮层宽度跟随视口收窄，空间不足时自动翻转到另一侧；窄屏下图片与字号自动收紧
...
> 数量 / 价格 / 库存都是**已格式化的字符串**，前端只做「数量 + 单位」的可翻译拼接（`_t("%(qty)s %(uom)s")`）。
...
| 触屏长按订单行 | 500ms 后弹出浮层；长按不会同时打开记录 |
