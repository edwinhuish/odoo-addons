...
| 新行按产品 id 缓存 | 新行的展示内容只由产品决定，所以取值签名就是 `product_id`：产品数据已缓存且产品没变时命中上一次结果（**悬停不发请求**），换了产品才重新装配；`onPatched` 时兜一次，保证换产品后立刻反映到浮层 |
...
| 新增产品行（未保存） | 正常弹浮层（悬停 / 触屏长按均可）；换成别的产品后浮层内容随之更新 |
...
> 验收日期：2026-09-17（T-014 已完成并归档，记录见 `CHANGELOG.md` →「验收记录（T-014）」）。
> 本清单同时作为**后续版本升级后的回归清单**。

| 验证项 | 期望 | 结果 |
|--------|------|------|
| `-u sale_product_hover` 升级 | 无报错，`/sale_product_hover/payload` 可访问 | 通过 |
| 报价单订单行悬停 | 浮层显示图片 / 名称 / 型号 / 规格 / 描述 / 可用库存，**无任何价格（无数量 / 单价 / 产品售价）** | 通过 |
| 销售订单订单行悬停 | 同报价单表现一致 | 通过 |
| 含变体产品 | 规格行显示 `颜色: 红, 尺寸: L` 之类；无变体产品的规格行不显示 | 通过 |
| 可用库存与单位 | 库存按 `Product Unit` 精度显示，并带产品计量单位名（如 `120 Units`） | 通过 |
| 移开与移入 | 移开延迟关闭；移入浮层不关闭；从浮层移回行可重开 | 通过 |
| 触屏长按 | 平板 / 手机长按订单行弹出浮层；滚动列表不弹出；长按不会打开记录 | 通过 |
| 窄屏 | 浮层宽度不超出屏幕，空间不足时自动换侧 | 通过 |
| 原有操作不受影响 | 点击进入、内联编辑、勾选、删除照旧；编辑态不弹浮层 | 通过 |
| 非目标列表 | 采购订单行、发票行不出现浮层 | 通过 |
| 边界情况 | 无图显示占位；服务类产品无库存行（且不留空的分隔线）；新行未选产品不弹 | 通过 |
| 新增产品行悬停 | 未保存的新行也能弹浮层，内容与保存后一致 | 通过 |
| 双语 | 默认英文；装 `zh_CN` 并切换后标签为「可用库存」 | 通过 |
| 控制台 | 无 JS 报错、无重复请求 | 通过 |
...
- **文档索引**（四件套，各有分工，改代码前先挑对应的看）：

  | 文档 | 看什么 |
  |------|--------|
  | [`README.md`](README.md)（本文） | 怎么用：功能、字段、安装、验证清单 |
  | [`CHANGELOG.md`](CHANGELOG.md) | 改了什么：逐版本「变更 / 影响 / 文档」 |
  | [`AGENTS.md`](AGENTS.md) | 不能破坏什么：约束 + 踩坑档案 + 文件职责 |
  | [`IMPLEMENTATION.md`](IMPLEMENTATION.md) | 为什么这么写：架构、数据流、核心要点、难点、坑点（现象 / 原因 / 规避） |
  | [`RETROSPECTIVE.md`](RETROSPECTIVE.md) | 走弯路的过程复盘、方法论与自校验 / 冒烟脚本 |
