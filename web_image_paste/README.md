...
- **超大图**：超过服务器 `max_file_upload_size` 时弹出明确报错，带出具体大小（文案随界面语言）
...
> 验收日期：2026-09-02，目标环境验证通过（T-003，落地版本 `19.0.1.0.0`）。
> `19.0.2.1.0`（2026-09-07）完成国际化（T-006）：源语言改为英文 + `i18n/zh_CN.po` 中英双语，目标环境已验证通过。
> `19.0.2.1.1`（2026-09-09，T-013）补应用列表（Apps）中文元数据（模块名 / 摘要 / 描述 + 分类「图片」），目标环境已验收通过；记录见 `CHANGELOG.md` →「验收记录（T-013）」。
...
| 复用原生链路 | 不改核心模板文件 | 通过 |
| 应用列表中文名（19.0.2.1.1） | 中文环境「应用」搜 `web_image_paste`，卡片标题显示「图片粘贴上传」，摘要与详情描述为中文，左侧分类显示「生产力 / 图片」；英文环境仍为英文 | 通过 |
